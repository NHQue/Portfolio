...
export default function Scene({ setBg }) {
  ...
  const { scene } = useGLTF('/GLTFs/RichardsLab.glb')
  const sculptureGroup = scene.getObjectByName('Model')
  ...
  useFrame((state) => {
    ...
    // // Log camera position when it changes
    // const cam = state.camera
    // if (
    //   prevCamPos.current.x !== cam.position.x ||
    //   prevCamPos.current.y !== cam.position.y ||
    //   prevCamPos.current.z !== cam.position.z
    // ) {
    //   console.log('Camera position:', cam.position.clone())
    //   prevCamPos.current = cam.position.clone()
    // }
  })
  ...
  return (
    <>
      {/* _Vector3 {x: -2.0473753843698907, y: -0.8592460943746503, z: 1.92761811243313} */}

      <PerspectiveCamera makeDefault position={[-2, -0.8, 1.9]} fov={75}>
